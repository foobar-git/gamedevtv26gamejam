...
// Debug utility for tracking and displaying multiple time metrics (play time, countdown,
// pause time). Keyboard shortcuts 1–9 control all timer functions.
// TODO: [Testing] - remove before shipping
public class TimerScript : MonoBehaviour
{
    private int _width, _height;
    private Rect _rectRight, _rectLeft, _rectInfo;
    private GUIStyle _guiStyleUpperLeft, _guiStyleUpperRight;
    ...
    // temp snapshots cumulativePauseGameTime while active so the cumulative calc stays correct across pause/resume cycles
    ...
    void OnGUI()
    {
        ...
        {
            _guiStyleUpperRight = new GUIStyle(GUI.skin.GetStyle("label"));
            _guiStyleUpperRight.alignment = TextAnchor.UpperRight;
            _guiStyleUpperLeft = new GUIStyle(GUI.skin.GetStyle("label"));
            _guiStyleUpperLeft.alignment = TextAnchor.UpperLeft;

            _guiStyleUpperRight.fontSize = 6 * (_width / 200);
            _guiStyleUpperLeft.fontSize = 6 * (_width / 200);

            ShowTime(Time.realtimeSinceStartup, "Real time since start  ", _rectRight, _guiStyleUpperRight);
            ShowTime(Time.time, "TimerClass ", _rectLeft, _guiStyleUpperLeft);
    ...
    void ContinuePlayTime(float t)
    {
        ...
        // adjusts the reference point so the elapsed-time formula resumes from current playTime
